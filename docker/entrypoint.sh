...
if [ "$SERVER_NAME" != "speedtest.demo.com" ]; then
  sed -i "s/speedtest.demo.com/$SERVER_NAME/g" /etc/apache2/sites-available/default-ssl.conf
fi

if [ "$WEBPORT" != "80" ]; then
  sed -i "s/^Listen 80\$/Listen $WEBPORT/g" /etc/apache2/ports.conf
  sed -i "s/*:80>/0.0.0.0:$WEBPORT>/g" /etc/apache2/sites-available/000-default.conf 
fi

if [ "$SSLPORT" != "443" ]; then
  sed -i "s/Listen 443\$/Listen $SSLPORT/g" /etc/apache2/ports.conf
  sed -i "s/*:443>/0.0.0.0:$SSLPORT>/g" /etc/apache2/sites-available/default-ssl.conf 
fi
